...
/// Mutable device input used to create or update a [Device].
@unfreezed
abstract class DeviceInput with _$DeviceInput {
  const DeviceInput._();

  /// Constructs a mutable [DeviceInput].
  factory DeviceInput({
  ...
  /// Deserializes a [DeviceInput] from a JSON map.
  ...
  Future<StandardResponse<Device>> save({
    ...
    try {
      final response = await connector.mutate(
        GqlMutation(
          ...
          variables: [
            ...
            if (appId != null)
              GqlVariable(name: 'appId', value: appId, type: .id),
          ],
          fields: [
            GqlField(
                name: _getSaveMutationNameFromVariant(
                  variant,
                  isAdd: id == null,
                ),
                args: {
      ...
      Log.critical(
        "layrz_sdk/DeviceInput/save(): General exception => $err\n$stack",
      );
  ...
  static String _getSaveMutationNameFromVariant(
