...
/// Immutable device data for the Layrz platform.
///
/// A [Device] represents a physical or virtual data source — a GPS tracker, a Zigbee hub, a Modbus
/// gateway, or a peripheral — that reports [DeviceTelemetry] and can execute [DeviceCommand]s.
@freezed
abstract class Device with _$Device {
  const Device._();

  /// Constructs an immutable [Device].
  ...
  /// Deserializes a [Device] from a JSON map.
  ...
  static Future<List<Device>> fetchAll({
    ...
    try {
      ...
      if (response.status != .ok) {
        Log.warning("layrz_sdk/Device/fetchAll(): API returned status ${response.status}");
      ...
      Log.critical("layrz_sdk/Device/fetchAll(): General exception => $err\n$stack");
  ...
  Future<Device?> fetch({
    ...
    try {
      ...
      if (response.status != .ok) {
        Log.warning("layrz_sdk/Device/fetch(): API returned status ${response.status}");
        onResponse?.call(response.status);
        return null;
      }
      ...
      Log.critical("layrz_sdk/Device/fetch(): General exception => $err\n$stack");
      onResponse?.call(.internalError);
      return null;
    }
  }
  ...
  Future<bool> delete({
    ...
    try {
      ...
      if (response.status != .ok) {
        Log.warning("layrz_sdk/Device/delete(): API returned status ${response.status}");
        onResponse?.call(response.status);
        return false;
      }
      ...
      Log.critical("layrz_sdk/Device/delete(): General exception => $err\n$stack");
      onResponse?.call(.internalError);
      return false;
    }
  }
  ...
  static Future<bool> deleteMany({
    ...
    try {
      ...
      if (response.status != .ok) {
        Log.warning("layrz_sdk/Device/deleteMany(): API returned status ${response.status}");
        onResponse?.call(response.status);
        return false;
      }
      ...
      Log.critical("layrz_sdk/Device/deleteMany(): General exception => $err\n$stack");
      onResponse?.call(.internalError);
      return false;
    }
  }
  ...
  static Future<List<Device>> fetchZigbeeCapable({
    ...
    try {
      ...
      if (data == null) {
        ...
        Log.error("layrz_sdk/Device/fetchZigbeeCapable(): No response from server");
        return [];
      }
      ...
      if (result == null) {
        onResponse?.call(ApiStatus.internalError.toJson());
        Log.error("layrz_sdk/Device/fetchZigbeeCapable(): No result from server");
        return [];
      }
      ...
      Log.critical("layrz_sdk/Device/fetchZigbeeCapable(): General exception => $e\n$stack");
      return [];
    }
  }
  ...
  static Future<bool> importToApp({
    ...
    try {
      ...
      if (response.status != .ok) {
        Log.warning("layrz_sdk/Device/importToApp(): API returned status ${response.status}");
        onResponse?.call(response.status);
        return false;
      }
      ...
      Log.critical("layrz_sdk/Device/importToApp(): General exception => $e\n$stack");
      onResponse?.call(.internalError);
      return false;
    }
  }
