...
/// Mutable chart input used to create or update a [LayrzChart].
@unfreezed
abstract class LayrzChartInput with _$LayrzChartInput {
  const LayrzChartInput._();

  /// Constructs a mutable [LayrzChartInput].
  factory LayrzChartInput({
    ...
    @Default(ChartDataSource.messages)
  ...
  /// Deserializes a [LayrzChartInput] from a JSON map.
  factory LayrzChartInput.fromJson(Map<String, dynamic> json) =>
      _$LayrzChartInputFromJson(json);

  // coverage:ignore-start
  /// [save] saves the chart input to the API, creating a new chart when [id] is null or editing the existing one
  /// otherwise.
  Future<ApiResponse<LayrzChart, Map<String, dynamic>>?> save({
    ...
    try {
      ...
      if (data == null) {
        ...
        Log.error("layrz_sdk/LayrzChartInput/save(): No response from server");
        return null;
      }
      ...
      if (result == null) {
        onResponse?.call(ApiStatus.internalError.toJson());
        Log.error("layrz_sdk/LayrzChartInput/save(): No result from server");
        return null;
      }
      ...
      return ApiResponse(
        status: ApiStatus.ok,
        result: LayrzChart.fromJson(result['result']),
      );
    } catch (e, stack) {
      Log.critical(
        "layrz_sdk/LayrzChartInput/save(): General exception => $e\n$stack",
      );
      return null;
    }
  }
