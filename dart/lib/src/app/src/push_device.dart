...
abstract class PushDevice with _$PushDevice {
  /// Constructs an immutable [PushDevice].
  ...
  /// Deserializes a [PushDevice] from a JSON map.
  ...
  static GqlFragment get gqlFragment =>
      GqlFragment(name: 'pushDeviceFragment', onType: 'PushDevice')
    ...
    try {
      final response = await connector.query(
        ...
        (json) {
          ...
          if (resultMap == null) {
            Log.warning(
              "layrz_sdk/PushDevice/validate(): No result in response",
            );
            return null;
          }
          ...
          if (deviceJson == null) {
            Log.warning("layrz_sdk/PushDevice/validate(): No device in result");
            return null;
          }

          final device = PushDevice.fromJson(deviceJson);
          final operationIds =
              (resultMap['operationIds'] as List<dynamic>?)
                  ?.map((e) => e as String)
                  .toList() ??
              [];

          return PushDeviceValidation(
            device: device,
      ...
      Log.critical(
        "layrz_sdk/PushDevice/validate(): General exception => $e\n$stack",
      );
      return null;
    }
  }
    ...
    try {
      final response = await connector.mutate(
        ...
        (json) {
          ...
          if (deviceId == null) {
            Log.warning(
              "layrz_sdk/PushDevice/register(): Device ID missing in result",
            );
            return null;
          }
          return deviceId;
      ...
      Log.critical(
        "layrz_sdk/PushDevice/register(): General exception => $e\n$stack",
      );
      return null;
    }
  }
  ...
  static Future<bool> bindOperations({
    ...
    try {
      ...
      Log.critical(
        "layrz_sdk/PushDevice/bindOperations(): General exception => $e\n$stack",
      );
      return false;
    }
  }
    ...
    try {
      final response = await connector.mutate(
        GqlMutation(
          ...
          GqlField(name: 'deregisterPushDevice', args: {'deviceId': 'deviceId'})
            ..add(GqlField(name: 'status')),
        ),
      );
      ...
      Log.critical(
        "layrz_sdk/PushDevice/deregister(): General exception => $e\n$stack",
      );
      return false;
    }
  }
