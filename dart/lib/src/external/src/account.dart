...
///
/// [source] references [ExternalSource], the supported external integration source/protocol.
@freezed
abstract class ExternalAccount with _$ExternalAccount {
  /// Constructs an immutable [ExternalAccount].
  ...
  static Future<ExternalAccount?> fetch({
    ...
    try {
      ...
      if (response.status != ApiStatus.ok) {
        ...
        Log.error("layrz_sdk/ExternalAccount/fetch(): ${response.status} => ${response.errors}");
        return null;
      }
      ...
      Log.critical("layrz_sdk/ExternalAccount/fetch(): General exception => $e\n$stack");
      return null;
    }
  }
  ...
  static Future<List<ExternalAccount>> fetchAll({
    ...
    try {
      ...
      if (response.status != ApiStatus.ok) {
        onResponse?.call(response.status.toJson());
        Log.error("layrz_sdk/ExternalAccount/fetchAll(): ${response.status} => ${response.errors}");
        return [];
      }

      return response.result ?? [];
    } catch (e, stack) {
      Log.critical("layrz_sdk/ExternalAccount/fetchAll(): General exception => $e\n$stack");
      return [];
    }
  }
  ...
  static Future<bool> deleteMany({
    ...
    try {
      ...
      if (response.status != ApiStatus.ok) {
        onResponse?.call(response.status.toJson());
        Log.error("layrz_sdk/ExternalAccount/deleteMany(): ${response.status} => ${response.errors}");
        return false;
      }

      return true;
    } catch (e, stack) {
      Log.critical("layrz_sdk/ExternalAccount/deleteMany(): General exception => $e\n$stack");
      return false;
    }
  }
  // coverage:ignore-end
...
abstract class ExternalAccountInput with _$ExternalAccountInput {
  ...
  /// Constructs a mutable [ExternalAccountInput].
  ...
  Future<ApiResponse<ExternalAccount, Map<String, dynamic>>?> save({
    ...
    try {
      ...
      if (response.status != ApiStatus.ok) {
        onResponse?.call(response.status.toJson());
        Log.error("layrz_sdk/ExternalAccountInput/save(): ${response.status} => ${response.errors}");
        return ApiResponse(
      ...
      Log.critical("layrz_sdk/ExternalAccountInput/save(): General exception => $e\n$stack");
      return null;
    }
  }
  // coverage:ignore-end
