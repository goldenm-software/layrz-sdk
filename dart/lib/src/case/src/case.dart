...
/// Immutable case within a system, encapsulating various details and attributes such as its
/// status, the asset and trigger involved, and the discussion thread attached to it.
@freezed
abstract class Case with _$Case {
  /// Constructs an immutable [Case].
  const factory Case({
  ...
  /// Deserializes a [Case] from a JSON map.
...
enum CaseStatus {
  ...
  /// Returns the JSON representation of this status value.
  @override
  String toString() => toJson();

  /// Converts this [CaseStatus] to its wire JSON string.
  ///
  /// Returns the `@JsonValue` annotation for this enum member. If the enum value is not found in
  /// the generated map (should never occur), defaults to `'PENDING'`.
  String toJson() => _$CaseStatusEnumMap[this] ?? 'PENDING';

  /// Converts a JSON string to a [CaseStatus].
  ///
  /// If the input does not match any known status, returns [pending] as the fallback default.
...
/// Whether a [Case] is in its normal state or has been marked for special handling.
@JsonEnum(alwaysCreate: true)
enum CaseIgnoredStatus {
  /// [normal] indicates that the case is in its default state and has not been marked for any
  /// special handling.
  @JsonValue('NORMAL')
  normal,

  /// [ignored] means that the case has been deliberately set aside and will not be considered
  /// for further action or review.
  @JsonValue('IGNORED')
  ignored,

  /// [preset] suggests that the case has been pre-configured or set up in advance, possibly with
  /// specific parameters or conditions.
  @JsonValue('PRESET')
  preset,

  /// [expired] indicates that the case is no longer valid or active due to the passage of time
  /// or failure to meet certain conditions.
  @JsonValue('EXPIRED')
  expired,

  /// [auto] implies that the case has been automatically generated or modified by a system or
  /// algorithm without manual intervention.
  @JsonValue('AUTO')
  auto;

  /// Returns the JSON representation of this ignored status value.
  @override
  String toString() => toJson();

  /// Converts this [CaseIgnoredStatus] to its wire JSON string.
  ///
  /// Returns the `@JsonValue` annotation for this enum member. If the enum value is not found in
  /// the generated map (should never occur), defaults to `'NORMAL'`.
  String toJson() => _$CaseIgnoredStatusEnumMap[this] ?? 'NORMAL';

  /// Converts a JSON string to a [CaseIgnoredStatus].
  ///
  /// If the input does not match any known ignored status, returns [normal] as the fallback
  /// default.
