...
/// HTTP-style status codes for Layrz API responses.
///
/// [ApiStatus] enumerates all possible outcome states for GraphQL operations,
/// from success ([ok]) to various error conditions (authentication failures,
/// resource not found, rate limiting, etc.). Each value is serialized to
/// JSON as an uppercase string (e.g., `OK`, `NOT_FOUND`).
///
/// Convert between enum and JSON using [toJson] and [fromJson]; the latter
/// tolerantly accepts both legacy squashed formats (e.g., `NOTFOUND`) and
/// modern underscored formats (e.g., `NOT_FOUND`).
...
enum ApiStatus {
  ...
  /// Returns the JSON representation of this status value (e.g., `OK`,
  /// `NOT_FOUND`).
  @override
