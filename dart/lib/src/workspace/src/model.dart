...
/// The top-level configuration for a Layrz workspace dashboard.
///
/// A [Workspace] groups the assets and services a user monitors, together
/// with the per-monitor configuration (cases, checkpoints, map, analytics,
/// sensors grid, table) that is active only when [typeApp] is
/// [AppInternalIdentifier.one].
///
/// References [InboundService], [OutboundService], [Trigger], [Checkpoint],
/// [Geofence], [GeoPoint], [LayrzChart], and [Timezone], which are owned by
/// other modules and are not defined here. [Asset] and [Access] are also
/// owned by other modules but ARE already available in the SDK.
@freezed
abstract class Workspace with _$Workspace {
  /// Constructs an immutable [Workspace].
...
abstract class WorkspaceInput with _$WorkspaceInput {
  ...
  factory WorkspaceInput({
    ...
    @Default(Color(0xFF2196F3)) @ColorOrNullConverter() Color? color,
    @IconOrNullConverter() MdiRemapIcon? icon,
    String? background,
  ...
  /// Deserializes a [WorkspaceInput] from a JSON map.
