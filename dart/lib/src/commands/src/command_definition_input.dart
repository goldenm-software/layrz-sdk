...
/// Mutable command definition input used to build or edit a [CommandDefinition].
@unfreezed
abstract class CommandDefinitionInput with _$CommandDefinitionInput {
  const CommandDefinitionInput._();

  /// Constructs a mutable [CommandDefinitionInput].
  ...
  /// Deserializes a [CommandDefinitionInput] from a JSON map.
  ...
  static Future<List<CommandDefinitionInput>> fetchFromFlespi({
    ...
    try {
      ...
      if (response.status != .ok) {
        Log.warning('layrz_sdk/CommandDefinitionInput/fetchFromFlespi(): API returned status ${response.status}');
      ...
      Log.critical(
        'layrz_sdk/CommandDefinitionInput/fetchFromFlespi(): Error fetching command definitions: $err\n$stack',
      );
  ...
  static Future<List<CommandDefinitionInput>> fetchFromFlespiFromModel({
    ...
    try {
      ...
      if (response.status != .ok) {
        Log.warning(
          'layrz_sdk/CommandDefinitionInput/fetchFromFlespiFromModel(): API returned status ${response.status}',
        );
        onResponse?.call(response.status);
        return [];
      }
      ...
      Log.critical(
        'layrz_sdk/CommandDefinitionInput/fetchFromFlespiFromModel(): '
        'Error fetching command definitions: $err\n$stack',
      );
      onResponse?.call(.internalError);
      return [];
    }
