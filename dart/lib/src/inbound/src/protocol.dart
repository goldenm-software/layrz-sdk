...
abstract class InboundProtocol with _$InboundProtocol {
  ...
  static Future<List<InboundProtocol>> fetchAll({
    ...
    try {
      ...
      if (response.status != .ok) {
        Log.warning('layrz_sdk/InboundProtocol/fetchAll(): API returned status ${response.status}');
      ...
      Log.critical('layrz_sdk/InboundProtocol/fetchAll(): Error fetching protocols: $err\n$stack');
  ...
  Future<InboundProtocol?> fetch({
    ...
    try {
      ...
      if (response.status != .ok) {
        Log.warning('layrz_sdk/InboundProtocol/fetch(): API returned status ${response.status}');
        onResponse?.call(response.status);
        return null;
      }
      ...
      Log.critical('layrz_sdk/InboundProtocol/fetch(): Error fetching protocol: $err\n$stack');
      onResponse?.call(.internalError);
      return null;
    }
  }
