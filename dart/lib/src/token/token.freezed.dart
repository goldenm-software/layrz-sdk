...
/// Unique identifier for the token (typically a UUID or similar).
 String? get id;/// The opaque token string used for authentication.
/// **Security**: Keep this value confidential; never log or persist it in plaintext.
 String get token;/// The date and time (UTC) until which this token is valid.
/// Null indicates the token does not expire.
@TimestampOrNullConverter() DateTime? get validBefore;/// The date and time (UTC) when this token was issued.
/// Null if not tracked by the server.
@TimestampOrNullConverter() DateTime? get issuedAt;/// The intended audience for this token (e.g., [TokenAudience.api] or
