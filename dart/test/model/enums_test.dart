...
void main() {
  ...
  group('Enum programmatic iteration', () {
    ...
    test('RenderWidget.values all have unique wire values', () {
      ...
      expect(wires.length, uniqueWires.length, reason: 'Found duplicate wire values in RenderWidget');
    });
    ...
    test('ZigbeeDataType.values all have unique wire values', () {
      ...
      expect(wires.length, uniqueWires.length, reason: 'Found duplicate wire values in ZigbeeDataType');
    });
    ...
    test('Each RenderWidget can be deserialized from its wire value', () {
      for (final widget in RenderWidget.values) {
        ...
        expect(deserialized, widget, reason: 'Deserialization failed for wire: $wire');
      }
    });
  });
}
